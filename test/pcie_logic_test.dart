...
void main() {
  group('PCIeSpeedLogic', () {
    test('toSpeedStr returns correct numeric value for Gen3 x16', () {
      final s = PCIeSpeedLogic.toSpeedStr(3, 16);
      expect(s, isNotEmpty);
    ...
    test('toSpeedStr returns empty string for null or unknown inputs', () {
      expect(PCIeSpeedLogic.toSpeedStr(null, 16), '');
      expect(PCIeSpeedLogic.toSpeedStr(3, null), '');
      expect(PCIeSpeedLogic.toSpeedStr(0, 1), '');
    });

    test('toSpeedStr formats output with two decimals', () {
      final s = PCIeSpeedLogic.toSpeedStr(7, 1); // 15.125 -> should format
      final numStr = s.replaceAll(RegExp('[^0-9.]'), '');
